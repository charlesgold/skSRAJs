...
function createCookie(e, t, n) {
    ...
    document.cookie = e + "=" + t + i + "; path=/; domain=." + window.location.hostname
}
...
function resizeIframe(e) {
    document.getElementById("beesocial").height = parseInt(e) + 10
}

function checkUrl() {
    if (!jQueryLoaded()) return !1;
    getParam("fb_ref") && fb_ref_regex.test(getParam("fb_ref")) && (createCookie("beetailer_ref", getParam("fb_ref"), 720), createCookie("beetailer_ref_date", (new Date).getTime() / 1e3, 720)), readCookie("beetailer_ref") && jQuery("form[action='/cart']").append('<input type="hidden" name="attributes[url]" value="' + window.location.hostname + '" /><input type="hidden" name="attributes[beetailer_ref]" value="' + readCookie("beetailer_ref") + '" /><input type="hidden" name="attributes[beetailer_ref_date]" value="' + readCookie("beetailer_ref_date") + '" />')
}

function hidePromoPopup() {
    var e = document.getElementsByTagName("html")[0],
        t = document.getElementById("beetailer_promotion");
    e.removeChild(t), createCookie("bt_hide_promos_" + MD5(document.domain), 1, 1)
}

function addPromos() {
    if (!readCookie("bt_hide_promos_" + MD5(document.domain))) {
        var e = document.getElementsByTagName("head")[0],
            t = document.createElement("script");
        t.type = "text/javascript", src = "//www.beetailer.com/out/promotions.js?domain=" + window.location.hostname + "&locale=" + getBrowserLanguage(), t.src = src + "&hash=" + MD5(src), e.appendChild(t)
    }
}

function addBeesocial() {
    typeof bt_widget_label == "undefined" && (bt_widget_label = "beesocial");
    if (document.getElementById(bt_widget_label) && !document.getElementById("social_widget_iframe")) {
        ...
        var e = document.getElementById(bt_widget_label).attributes,
            t = Array(),
            n = {
                "data-domain": "domain",
                "data-product-id": "product_id",
                "data-url": "url",
                "data-comment-width": "data_comment_width",
                "data-css-style": "data_css_style",
                "data-disable-twitter": "data_disable_twitter",
                "data-disable-like": "data_disable_like",
                "data-disable-send": "data_disable_send",
                "data-disable-comment": "data_disable_comment",
                "data-disable-plusone": "data_disable_plusone",
                "data-disable-pinterest": "data_disable_pinterest",
                "data-twitter-text": "data_twitter_text",
                "data-fb-comment-style": "data_fb_comment_style",
                "data-fb-comment-num-post": "data_fb_comment_num_post",
                "data-product-image": "data_product_image",
                platform: "platform",
                "data-disable-fancy": "data_disable_fancy",
                "data-disable-wanelo": "data_disable_wanelo",
                "data-product-title": "title",
                uuid: "uuid",
                "data-pro-wm-removal": "data_pro_wm_removal",
                "data-pro-notif-comments": "data_pro_notif_comments",
                "data-pro-fb-share": "data_pro_fb_share"
            },
            r = "?locale=" + getBrowserLanguage();
        for (i = 0; i < e.length; i++) n[e[i].nodeName] && (r += "&" + n[e[i].nodeName] + "=" + encodeURIComponent(e[i].nodeValue));
        r += "&data_comment_width=" + document.getElementById("beesocial").clientWidth;
        var s = document.createElement("iframe");
        s.setAttribute("frameBorder", "0"), s.setAttribute("scrolling", "no"), s.setAttribute("width", "100%"), s.setAttribute("height", "1"), s.setAttribute("id", "social_widget_iframe");
        var o = document.location.protocol + "//www.beetailer.com/out/social_widget" + r;
        s.src = o + "&hash=" + MD5(o) + "#" + window.location.href, label.appendChild(s), XD.receiveMessage(function(e) {
            document.getElementById("social_widget_iframe").style.height = e.data + "px"
...
function initializeOnLoad() {
    addBeesocial(), checkUrl(), addPromos()
}
typeof Shopify != "undefined" && (Shopify.queue = [], Shopify.moveAlong = function() {
    if (Shopify.queue.length) {
        var e = Shopify.queue.shift();
        Shopify.addItem(e.variant_id, e.quantity, function() {
            Shopify.moveAlong()
        })
    } else window.location = "http://" + window.location.hostname + "/cart"
});
var fb_ref_regex = /^beetailer_(.*)/;
jQueryLoaded() && jQuery(document).ready(function() {
    var e = getParam("checkout_hash");
    typeof e != "undefined" && e != "" && jQuery.ajax({
        url: "https://www.beetailer.com/shopify/cart/checkout?checkout_hash=" + e + "&callback=?",
        dataType: "json",
        success: function(e) {
            Shopify.queue = e.items, Shopify.moveAlong()
        }
    })
});
var XD = function() {
        ...
        return {
            postMessage: function(e, t, r) {
                if (!t) return;
                r = r || parent, i.postMessage ? r.postMessage(e, t.replace(/([^:]+:\/\/[^\/]+).*/, "$1")) : t && (r.location = t.replace(/#.*$/, "") + "#" + +(new Date) + n++ + "&" + e)
            },
            receiveMessage: function(n, s) {
                i.postMessage ? (n && (r = function(e) {
                    if (typeof s == "string" && typeof e.origin == "string" && e.origin.replace(/http(s*):\/\//, "") !== s.replace(/http(s*):\/\//, "") || Object.prototype.toString.call(s) === "[object Function]" && s(e.origin) === !1) return !1;
                    n(e)
                }), i.addEventListener ? i[n ? "addEventListener" : "removeEventListener"]("message", r, !1) : i[n ? "attachEvent" : "detachEvent"]("onmessage", r)) : (e && clearInterval(e), e = null, n && (e = setInterval(function() {
...
window.addEventListener ? window.addEventListener("load", initializeOnLoad, !1) : window.attachEvent && window.attachEvent("onload", initializeOnLoad), initializeOnLoad();